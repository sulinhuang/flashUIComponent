	...
	public class VScrollBar extends UIComponent
	{
		...
		public function setTarget(target:DisplayObject,enableHandDrag:Boolean=false, clipWidth:Number=0, clipHeight:Number=0,clipX:Number=0,clipY:Number=0):void {
			...
			if (enableHandDrag == true || UIComponent.isMobile == true) 
			{
				...
				if(target.parent && target.parent is List){
		...
		public function addMouseWheelScroll(sp:InteractiveObject):void
		{
			if(_target){
				_target.removeEventListener(MouseEvent.MOUSE_WHEEL,mouseWellScroll);
			...
			sp.addEventListener(MouseEvent.MOUSE_WHEEL,mouseWellScroll);
			sp.addEventListener(MouseEvent.ROLL_OUT, setMyAlphaOut);
			sp.addEventListener(MouseEvent.ROLL_OVER, setMyAlphaIn);
		}
		
		public function initMouseOverOut(target:DisplayObject):void
		{
			target.addEventListener(MouseEvent.ROLL_OUT, setMyAlphaOut);
			target.addEventListener(MouseEvent.ROLL_OVER, setMyAlphaIn);
		}
...
//			newSize += 2;
		...
		public var isCheckOutside:Boolean = true;
		...
		protected function stopDragScroller(event:MouseEvent=null):void 
		{
			if(isDargDisableStageMouseChildren == true)
			{
			...
			_isMouseDrag = false;
			if(this.stage == null){
				UIComponent.stage.removeEventListener(MouseEvent.MOUSE_MOVE, updateToPos);
				UIComponent.stage.removeEventListener(MouseEvent.MOUSE_UP, stopDragScroller);
				return;
			}
		...
		protected function scroll(event:Event=null):void 
		{
			...
			if(_target != null)
			{
				...
				var isOnePix:Boolean = Math.abs(toY - getRectPos())<1.01;
		...
		public var isscrollToPositionByMotionWhenUserDrag:Boolean = true;
		
		public function scrollToPositionByMotion(pos:int):void
		{
			if(_isDraging==true) return;
			if(isscrollToPositionByMotionWhenUserDrag == true){
				try
				{
					if(isDargDisableStageMouseChildren == true)
					{
		...
		public function updateMaxPos():void 
		{
			if(isTextField)
			{
