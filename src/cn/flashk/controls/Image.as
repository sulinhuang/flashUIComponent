...
//			clear();
		...
		{
			trace("Image IOError:",event.text);
			this.dispatchEvent(new Event("imageerror"));
		}
		...
		protected function loadFile(filePath:String):void
		{
            ...
            {
			    ldr = new Loader();
				ldr.contentLoaderInfo.addEventListener(IOErrorEvent.IO_ERROR,onImageIOError);
		...
		public var imageNoScale:Boolean = false;
		
		protected function alignBP():void
		{
			if(_bp == null) return;
			if(imageNoScale == true) return;
			_bp.smoothing = smoothing;
			...
			if(zoomInContent == true && (_bp.width<maxW || _bp.height <maxH))
			{
				...
				this.scrollRect = new Rectangle(0,0,_compoWidth,_compoHeight);
			}
